...
class AgeComponent:
    ...
    def __init__(self, age_years: float, age_profile_spec: Any, default_units: str = "years"):
    ...
    def _normalize_age_profile(self, spec: Any) -> list[Dict[str, Any]]:
        ...
        if isinstance(spec, dict):
            self.age_units = spec.get("units", "years") if isinstance(spec.get("units"), str) else "years"
...
class MetabolismComponent:
    ...
    def __init__(self, cfg: Dict[str, Any], initial_calories: float = 0.0, initial_max: float = 0.0, initial_body_nutrition: float = 0.0):
        ...
        self.carcass_edible_ratio = self._coerce_positive_float(cfg.get("carcass_edible_ratio"), fallback=0.55)
        self.carcass_calories_per_kg = self._coerce_positive_float(cfg.get("carcass_calories_per_kg"), fallback=1800.0)
    ...
    def _resolve_daily_calorie_need(self, cfg: Dict[str, Any]) -> float:
        explicit = self._coerce_positive_float(cfg.get("daily_calorie_need"), fallback=0.0)
        if explicit > 0.0:
            return explicit
        intake_kg = self._coerce_positive_float(cfg.get("daily_food_intake_kg"), fallback=0.0)
        cals = self._coerce_positive_float(cfg.get("food_calories_per_kg"), fallback=0.0)
        if intake_kg > 0.0 and cals > 0.0:
            return intake_kg * cals
        return 2400.0

    def _resolve_meal_calories(self, cfg: Dict[str, Any]) -> float:
        explicit = self._coerce_positive_float(cfg.get("meal_calories"), fallback=0.0)
        if explicit > 0.0:
            return explicit
        meal_kg = self._coerce_positive_float(cfg.get("meal_intake_kg"), fallback=0.0)
        cals = self._coerce_positive_float(cfg.get("food_calories_per_kg"), fallback=0.0)
        if meal_kg > 0.0 and cals > 0.0:
            return meal_kg * cals
        return 60.0

    def apply_profile(self, metabolism_cfg: Dict[str, Any], age_stage: str, sex: str, traits: Dict[str, Any]) -> None:
