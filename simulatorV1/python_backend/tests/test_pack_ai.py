...
from simulation.step_context import (
...
def _build_hunter(
    ...
    *,
    ...
    """Construit rapidement un carnivore de test avec une
    configuration de chasse simple.
    """
...
class PackAITestCase(unittest.TestCase):
    ...
    def test_target_filter_keeps_requested_species(self) -> None:
        ...
        gazelle = Animal(
            "Gazelle", (60.0, 0.0), diet="herbivore", species_type="gazelle"
        )
        elephant = Animal(
            "Elephant", (10.0, 0.0), diet="herbivore", species_type="elephant"
        )
    ...
    def test_same_priority_pack_members_can_all_feed(self) -> None:
        ...
        for animal in animals:
            acted, _, resolve_food = execute_predation_cycle(
                animal, animals, world, lambda _msg: None
            )
        ...
        self.assertSetEqual(
            fed_animals,
            {leader.animal_id, hunter_a.animal_id, hunter_b.animal_id},
        )
    ...
    def test_pack_members_follow_shared_target(self) -> None:
        ...
        gazelle_primary = Animal(
            "GazelleA", (100.0, 0.0), diet="herbivore", species_type="gazelle"
        )
        gazelle_secondary = Animal(
            "GazelleB", (0.0, 120.0), diet="herbivore", species_type="gazelle"
        )
        animals = [leader, hunter, gazelle_primary, gazelle_secondary]

        acted, action, _ = execute_predation_cycle(
            leader, animals, world, lambda _msg: None
        )
        self.assertTrue(acted)
        ...
        acted, action, _ = execute_predation_cycle(
            hunter, animals, world, lambda _msg: None
        )
        self.assertTrue(acted)
        self.assertIn(action, {"pack_tracking_prey", "pack_closing_distance"})
    ...
    def test_blocked_thirst_does_not_fake_move_to_water(self) -> None:
        ...
        acted, action, _ = handle_thirst(
            animal, BlockedWaterWorld(), lambda _msg: None
        )
    ...
    def test_blocked_group_avoidance_does_not_report_fake_action(self) -> None:
        ...
        acted, action = maintain_group_cohesion(
            leader, [leader, follower], TrappedWorld(), spacing=20.0
        )
    ...
    def test_relationships_prioritize_hunt_before_territory(self) -> None:
        ...
        hunter.set_trait(
            "territory",
            {"center": (100.0, 100.0), "radius": 12.0, "margin": 6.0},
        )
        prey = Animal(
    ...
    def test_relationships_suspend_territory_when_hunger_is_critical(
        ...
        animal.set_trait(
            "territory",
            {"center": (100.0, 100.0), "radius": 20.0, "margin": 5.0},
        )
        animal.hunger = 90.0
    ...
    def test_engine_logs_terminal_snapshot_for_prey_killed_mid_step(
        ...
        prey = Animal(
            ...
            diet="herbivore",
            species_type="gazelle",
            body_nutrition=140.0,
        )

        engine = SimulationEngine(
            world, [hunter, prey], steps=1, verbose=False
        )
        ...
        prey_status = next(
            item
            for item in step["species"]
            if item["animal_id"] == prey.animal_id
        )
    ...
    def test_thirst_drinks_immediately_when_already_on_shore(self) -> None:
        ...
        acted, action, motivation = handle_thirst(
            animal, world, lambda _msg: None
        )
    ...
    def test_thirst_reuses_memorized_drink_target(self) -> None:
        class StableWaterWorld:
            ...
            def find_drink_target(
                ...
                return (
                    (4.0, 0.0)
                    if self.find_drink_target_calls == 1
                    else (12.0, 0.0)
                )
    ...
    def test_find_drink_target_prefers_same_shore_side(self) -> None:
        ...
        target = world.find_drink_target(
            animal.x, animal.y, {"x": 11.0, "y": 11.0}, entity=animal
        )

        self.assertLessEqual(target[0], 9.0)
        self.assertTrue(
            world._has_water_neighbor(int(target[0]), int(target[1]))
        )
    ...
    def test_find_drink_target_cache_rebuilds_after_new_water_tiles(
        ...
        initial_target = world.find_drink_target(
            7.0, 10.0, {"body_id": "body_1", "x": 10.0, "y": 10.0}
        )
        ...
        rebuilt_target = world.find_drink_target(
            7.0, 10.0, {"body_id": "body_1", "x": 10.0, "y": 10.0}
        )

        self.assertTrue(
            world._has_water_neighbor(
                int(initial_target[0]), int(initial_target[1])
            )
        )
        self.assertTrue(
            world._has_water_neighbor(
                int(rebuilt_target[0]), int(rebuilt_target[1])
            )
        )
    ...
    def test_carcass_keeps_link_to_killed_animal(self) -> None:
        ...
        self.assertTrue(
            str(carcass["id"]).startswith(f"carcass_{prey.animal_id}_")
        )
        self.assertAlmostEqual(carcass["source_body_mass_kg"], 27.6)
        self.assertAlmostEqual(
            carcass["calories"], prey.estimate_carcass_calories()
        )
        self.assertEqual(
            carcass["metadata"]["source_animal_id"], prey.animal_id
        )
    ...
    def test_active_shared_kill_overrides_cycle_rest(self) -> None:
        ...
        self.assertIn(
            status["action"],
            {
                "pack_feed_from_carcass_and_ate",
                "pack_move_to_carcass",
                "pack_waiting_move",
                "pack_waiting_guard",
                "pack_feed_from_carcass",
            },
        )
    ...
    def test_process_species_prioritizes_critical_hunger_before_moderate_thirst(  # noqa: E501
        ...
        self.assertIn(
            status["action"],
            {"move_to_seen_food", "move_to_seen_food_and_ate"},
        )
    ...
    def test_scavenger_blocks_shared_carcass_after_repeated_failures(
        ...
        self.assertFalse(resolve)
        self.assertTrue(
            shared == {} or scout.animal_id in shared.get("blocked", set())
        )
    ...
    def test_same_seed_reproduces_same_summary(self) -> None:
        ...
        def _run_once(seed: int):
            ...
            simulation = SimulationEngine(
                world, species, steps=20, verbose=False, seed=seed
            )
            simulation.run()
